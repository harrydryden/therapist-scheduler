...
Justin`,
  ...
  'chase.closureRecommendationHours': {
    ...
    description:
      'Hours of continued silence AFTER the chase email before a thread is recommended for closure. This same window is then reused as the grace period before an unresponsive pre-booking is auto-cancelled (if enabled). With defaults, a pre-booking auto-cancels after roughly: Chase-After-Stale (72h) + this (48h, to closure) + this again (48h, to cancel) ≈ 7 days of total inactivity.',
    valueType: 'number',
    minValue: 24,
    maxValue: 336,
  ...
  'voucher.autoUnsubscribeEnabled': {
    category: 'weeklyMailing',
    label: 'Auto-Unsubscribe After Expired Vouchers',
    description:
      'When enabled, users who let the maximum number of consecutive voucher codes expire are removed from the mailing list and sent the final-notice email ("freeing up your spot"). When disabled, users keep their spot: strikes are still counted for visibility, but no final notice is sent and a fresh code is issued as normal.',
    valueType: 'boolean',
    defaultValue: false,
  },
  'voucher.maxStrikes': {
    ...
    description: 'Auto-unsubscribe users from mailing list after this many consecutive expired (unused) voucher codes. Only takes effect when Auto-Unsubscribe After Expired Vouchers is enabled.',
    valueType: 'number',
    minValue: 1,
    maxValue: 10,
    defaultValue: 3,
