...
class EmailProcessingServiceFacade {
  ...
  threadContainsInboundReplies(threadId: string, traceId: string, sinceMs?: number) {
    return emailIngestService.threadContainsInboundReplies(threadId, traceId, sinceMs);
  }
