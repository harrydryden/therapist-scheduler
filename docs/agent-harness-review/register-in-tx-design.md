...
## 11. Implementation notes (Phase 2 — `confirmed.ts`)

Phase 2 (step 6 above) is implemented: `confirmed.ts`'s single atomic `update()` now runs inside an explicit `prisma.$transaction`, wrapping the update together with the intent registrations for `slack_notify_confirmed`, `email_client_confirmation`, `email_therapist_confirmation` (all keyed WITH the post-update generation) and `therapist_freeze_sync` (keyed WITHOUT one, matching `transitionSideEffectsService.onConfirmed`).

- **Isolation: Read Committed (Prisma default), as approved in decision 1.** The concurrency safety is entirely the WHERE-clause preconditions on the single UPDATE plus PG's row lock — a concurrent confirmation blocks on the lock, then re-evaluates its WHERE against the committed row, matches 0 rows, and takes the P2025 → idempotent-skip path. The added INSERTs read no contested state, so Serializable would only add serialization-conflict aborts for no safety gain.

- **The P2025 failure-attribution path deliberately stays non-transactional.** Rather than catching P2025 inside the transaction and re-fetching via the `tx` client (which the design doc §5 floated as a minor consistency improvement), the implementation converts P2025 to a private `ConfirmPreconditionFailed` sentinel thrown out of the transaction callback; the outer catch then runs the existing re-fetch + reclassify logic against the non-transactional `prisma` client, byte-for-byte as before. This sidesteps any dependency on the interactive transaction remaining usable after a caught query error, and since a P2025 update matched 0 rows (no writes), the transaction rolling back on the sentinel throw is a no-op. Net behavior change is confined to the success path — exactly where the crash-window fix is needed. Documented here because it's a conscious deviation from the doc's §5 suggestion, chosen for lower risk on the hottest transition.

- **Verification:** `npm run typecheck` / `npm run lint` clean. Full backend suite: 1720 passed (up from 1712 post-Phase-1), 0 failures. New coverage: `register-in-tx-confirmed.test.ts` (all four effect types register inside the transaction on the success path; the generation-with vs generation-without keying per effect type; sendEmails=false / missing-therapist-email / slack-off gating; and no registration on either the idempotent-skip or P2025 failure-attribution paths). The two existing `confirmed.ts` tests (`lifecycle-confirmed-semantic-equality`, `transition-to-confirmed-race`) were updated to mock `$transaction` + `getNotificationSettings` + `registerInTransaction`; all their original assertions (semantic-equality skip, P2025 race recovery, post-update generation, notAlreadyAtTarget guard) still pass unchanged.

With Phase 2, finding #10 is closed for all three lifecycle transitions (confirm, cancel, complete). The appointment-creation `justintime_start` outbox was already correct; there are no remaining post-commit side-effect registration windows in the lifecycle.
